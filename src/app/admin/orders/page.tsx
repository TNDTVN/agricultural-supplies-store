...
export default function OrderPage() {
    ...
    const [generatingPDF, setGeneratingPDF] = useState<number | null>(null); // Track which order is generating
    ...
    const handleExportInvoice = async (order: Order) => {
        if (generatingPDF !== null) return; // Prevent multiple exports at once

        setGeneratingPDF(order.orderID); // Set generating state

        try {
        // Generate the PDF as a Blob
        const pdfDoc = pdf(
            <InvoicePDF
            order={{ ...order, orderDetails: order.orderDetails || [] } as Order & { orderDetails: OrderDetail[] }}
            />
        );
        ...
        link.download = `HoaDon_${order.orderID}.pdf`;
        ...
        setGeneratingPDF(null); // Reset state
        }
    };
    ...
    const columns: ColumnDef<Order>[] = useMemo(
        ...
        },
        {
            accessorFn: (row) =>
                row.orderDetails
                    ?.reduce(
                        (sum: number, detail) =>
                            sum + (detail.unitPrice || 0) * (detail.quantity || 0) * (1 - (detail.discount || 0)),
                        0
                    )
        ...
        },
        {
            ...
            <div className="flex gap-2">
                <Button className="bg-cyan-500 hover:bg-cyan-700" onClick={() => openModal(row.original)}>
                Chi tiết
                </Button>
                <Button
                className="bg-green-500 hover:bg-green-700"
                onClick={() => handleExportInvoice(row.original)}
                disabled={generatingPDF !== null} // Disable all buttons while generating
                >
                {generatingPDF === row.original.orderID ? "Đang tạo..." : "Xuất hóa đơn"}
                </Button>
            </div>
            ),
            enableSorting: false,
