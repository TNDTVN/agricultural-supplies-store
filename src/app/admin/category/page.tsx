...
import { ChangeEvent, useEffect, useMemo, useState } from "react";
    ...
    if (!confirm("Bạn có chắc chắn muốn xóa loại sản phẩm này?")) return;
        try {
            ...
            if (!response.ok) {
            const errorText = await response.text(); // Lấy thông báo từ backend
            alert(errorText); // Hiển thị thông báo lỗi từ backend
            return; // Thoát hàm mà không ném lỗi
            }
            alert("Xóa loại sản phẩm thành công!");
            fetchCategories(currentPage);
        } catch (error) {
            // Nếu có lỗi không mong muốn (ví dụ: mất kết nối), vẫn hiển thị alert
            const err = error as Error;
            alert(err.message || "Có lỗi xảy ra khi xóa loại sản phẩm!");
        }
    };
